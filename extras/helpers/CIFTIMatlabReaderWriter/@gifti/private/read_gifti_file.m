...
try
    ...
    unix(['wb_command -cifti-convert -from-gifti-ext ' filename ' ' filename '.dtseries.nii']);
    unix(['wb_command -nifti-information -print-xml ' filename '.dtseries.nii > ' filename '.xml']);
    try
        %t = xmltree(filename);
        t = xmltree([filename '.xml']);
    catch
